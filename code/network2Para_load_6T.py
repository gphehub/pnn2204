"""
Versions:
network2Para_load_6T.py:
Based on network2Para_load_6S.py.
Changed the activation function to Tanh.
Modifications are indicated by '#mod_6T'.

network2Para_load_6S.py:
Based on network2Para_load_5S.py.
...
network2Para_load_5S.py:
Based on network2Para_load_5R.py and network2Para_load_S.py.
Changed the activation function to sigmoid.
Modifications are indicated by '#mod_LDS' (yes! not  '#mod_LD5S').
...
class Network(object):
    ...
    def SGDmod(self, evaluation_data=None): #mod_LD #mod_LDR
        ...
        with open('disagreed_results.csv','w', newline='') as resultsave: #mod_LD
            ...
            writer.writerow(("NN1:",rateNN1)) #mod_LD6S
            writer.writerow(("NN1p:",rateNN1p)) #mod_LD6S
            writer.writerow(("NN2:",rateNN2)) #mod_LD6S
            writer.writerow(("NN2p:",rateNN2p)) #mod_LD6S
            writer.writerow(("Para:",ratePara)) #mod_LD6S
...
def activation_fn(z): #mod_4R
    """The sigmoid function."""
    # return 1.0/(1.0+np.exp(-z)) #mod_LDS #mod_6T
    """The ReLU function.""" #mod_4R
    # return np.where(z<0,0,z) #mod_4R #mod_LDS
    """The Tanh function."""
    return 2*sigmoid(2*z)-1 #mod_6T

def activation_fn_prime(z): #mod_4R
    """Derivative of the sigmoid function."""
    # return sigmoid(z)*(1-sigmoid(z)) #mod_LDS #mod_6T
    """Derivative of the ReLU function.""" #mod_4R
    # return np.where(z<0,0,1) #mod_4R #mod_LDS
    """Derivative of the Tanh function."""
    return 4*sigmoid(2*z)*(1-sigmoid(2*z)) #mod_6T
